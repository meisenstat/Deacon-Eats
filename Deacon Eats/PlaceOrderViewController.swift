...
//        otherView = UIView(frame: CGRect(x: 0, y: otherButton.frame.maxY + 20, width: view.frame.width, height: 300))
    ...
    //Alert the user that the order has been placed with the location, time limit and special instructions as applicable
    @IBAction func placeOrder(_ sender: Any) {
        var message = ""
        if (otherInstructions == "None" && selectedTime[0] > 0){
            message = "Your order was placed at \(restaurant) with a time limit of \(selectedTime[0]) hours and \(selectedTime[1]) minutes."
        }else if(otherInstructions == "None" && selectedTime[0] == 0){
            message = "Your order was placed at \(restaurant) with a time limit of \(selectedTime[1]) minutes."
        }else if(selectedTime[0] == 0){
            message = "Your order was placed at \(restaurant) with a time limit of \(selectedTime[1]) minutes. Your special instructions are \"\(otherInstructions)\""
        }else{
            message = "Your order was placed at \(restaurant) with a time limit of \(selectedTime[0]) hours and \(selectedTime[1]) minutes. Your special instructions are \"\(otherInstructions)\""
        }
        let alert = UIAlertController(title: "Order Placed", message: message, preferredStyle: .alert)
